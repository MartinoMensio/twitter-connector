...
def save_tweets_from_user_id(tweets: List[dict], user_id: int):
    ...
    for t in tweets:
        ...
        if "_id" not in t:
            ...
            t["_id"] = t["id"]
            to_save.append(t)
            tweets_collection.replace_one({"_id": t["_id"]}, t, upsert=True)
    ...
    timelines_collection.replace_one(
        {"_id": user_id},
        {"_id": user_id, "last_updated": now, "tweet_ids": updated_tweet_ids},
        upsert=True,
    )
...
def save_since_id_v2(user_id, since_id):
    item = {"_id": user_id, "since_id": since_id}
...
def save_tweets_v2(tweets):
    for t in tweets:
        if "_id" not in t:
            # this tweet is new
            t["_id"] = t["id"]
            tweets_collection_v2.replace_one({"_id": t["_id"]}, t, upsert=True)
    return True
